                    ...
                    <div class="col-sm-12" style="margin: 0;padding: 0">
                        <div class="box">
                            ...
                            <div style="width: 50%;float: right;text-align: center;">
                                <form method="post" action="${pageContext.request.contextPath}/taskRecord/taskRecordList">
                                    <input style="text-indent: 8;" name="username" value="${username}" placeholder="请输入用户名">
                                    <input style="text-indent: 8;" name="taskName" value="${taskName}" placeholder="请输入任务名">
                            ...
                            <div class="item">
                                <div class="item-responsive ">
                                    <table class="table">
                                        ...
                                        <tbody>
                                        <c:forEach items="${pageInfo.list}" var="taskRecord">
                                            <tr>
                                                <td>${taskRecord.username}</td>
                                                <td>${taskRecord.taskName}</td>
                                                <td>${taskRecord.taskContent}</td>
                                                <td>
                                                    <c:if test="${taskRecord.status == 0}">
                                                        未发布
                                                    </c:if>
                                                    <c:if test="${taskRecord.status == 1}">
                                                        发布
                                                    </c:if>
                                                </td>
                                                <td>
                                                    <fmt:formatDate value="${taskRecord.addTime}" pattern="yyyy-MM-dd" />
                                                </td>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <nav aria-label="Page navigation">
                                        <ul class="pagination">
                                            <li><a href="${pageContext.request.contextPath}/taskRecord/taskRecordList?pn=1">首页</a></li>
                                            <!--上一页-->
                                            <li>
                                                <c:if test="${pageInfo.hasPreviousPage}">
                                                    <a href="${pageContext.request.contextPath}/taskRecord/taskRecordList?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                                            ...
                                            <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
                                                ...
                                                <c:if test="${page_num != pageInfo.pageNum}">
                                                    <li><a href="${pageContext.request.contextPath}/taskRecord/taskRecordList?pn=${page_num}">${page_num}</a></li>
                                                </c:if>
                                            </c:forEach>
                                            <!--下一页-->
                                            <li>
                                                <c:if test="${pageInfo.hasNextPage}">
                                                    <a href="${pageContext.request.contextPath}/taskRecord/taskRecordList?pn=${pageInfo.pageNum+1}"
                                                       aria-label="Next">
                                                        <span aria-hidden="true">»</span>
                                                    </a>
                                                </c:if>
                                            </li>
                                            <li><a href="${pageContext.request.contextPath}/taskRecord/taskRecordList?pn=${pageInfo.pages}">尾页</a></li>
                                        </ul>
                                    </nav>
                                </div>
                            </div>
