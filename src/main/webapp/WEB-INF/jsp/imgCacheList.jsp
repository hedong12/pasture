                ...
                <div class="row">
                    <div class="col-sm-12" style="margin: 0;padding: 0">
                        <div class="box">
                            ...
                            <input type="button"  class="modalLink btn btn-success" value="添加宠物图片">
...
<%--                             添加表单 --%>
                            <div id="modal1" class="modal">
                                <div>
                                    <h2 style="margin: 15px;text-align: center;color: black">添加宠物图片</h2>
                                </div>
                                <form id="form0" enctype="multipart/form-data">
                                    ...
                                    <label class="addlabel">图片名称：</label><input class="addinput" type="text"  id="imgCacheName" placeholder="请输入图片名称" /><span class="spa spa2"></span><br />
                                    <label class="addlabel">状态：</label>
                                        <select class="addinput" id="imgCacheStatus">
                                            <option value="">
                                                请选择状态
                                            </option>
                                            <option value="0">
                            ...
                            <div style="width: 50%;float: right;text-align: center;">
                                <form method="post" action="${pageContext.request.contextPath}/imgcache/imgCacheList">
                                    <input  name="imgCacheName" value="${imgCacheName}" placeholder="请输入图片名...">
                            ...
                            <div class="item">
                                <div class="item-responsive ">
                                    <table class="table">
                                        ...
                                        <tbody>
                                        <c:forEach items="${pageInfo.list}" var="imgCache">
                                            <tr>
                                                <td>${imgCache.imgCacheName}</td>
                                                <td>
                                                    <img class="pic" height="40" width="60"  title="${imgCache.imgCacheName}" src="${pageContext.request.contextPath}/uploaded/${imgCache.imgCacheUrl}">
                                                </td>
                                                <td>
                                                    <c:if test="${imgCache.imgCacheStatus == 0}">
                                                        正常
                                                    </c:if>
                                                    <c:if test="${imgCache.imgCacheStatus == 1}">
                                                        禁用
                                                    </c:if>

                                                </td>
                                                <td>
                                                     <a  onclick="delFodderById('${imgCache.imgCacheId}')" href="javascript:;">
                                                         <span class="item-status delivered">删除</span>
                                                     </a>
                                                     <a id="updatePetsById" href="${pageContext.request.contextPath}/imgcache/toUpdateImgcacheById?id=${imgCache.imgCacheId}"><span class="item-status delivered">修改</span></a>
                                ...
                                <div class="col-md-6">
                                    <nav aria-label="Page navigation">
                                        <ul class="pagination">
                                            ...
                                            <li><a href="${pageContext.request.contextPath}/imgcache/imgCacheList?pn=1">首页</a></li>

                                            <!--上一页-->
                                            <li>
                                                <c:if test="${pageInfo.hasPreviousPage}">
                                                    <a href="${pageContext.request.contextPath}/imgcache/imgCacheList?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                                            ...
                                            <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
                                                ...
                                                <c:if test="${page_num != pageInfo.pageNum}">
                                                    <li><a href="${pageContext.request.contextPath}/imgcache/imgCacheList?pn=${page_num}">${page_num}</a></li>
                                                </c:if>
                                            </c:forEach>
                                            ...
                                            <li>
                                                <c:if test="${pageInfo.hasNextPage}">
                                                    <a href="${pageContext.request.contextPath}/imgcache/imgCacheList?pn=${pageInfo.pageNum+1}"
                                                       aria-label="Next">
                                                        <span aria-hidden="true">»</span>
                                                    </a>
                                                </c:if>
                                            ...
                                            <li><a href="${pageContext.request.contextPath}/imgcache/imgCacheList?pn=${pageInfo.pages}">尾页</a></li>
                                        </ul>
                                    </nav>
                                </div>
                            </div>
...
<script>
        ...
        function delFodderById(e) {
            ...
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/imgcache/delImgCacheById",
                data: {"id" : e},
                dataType: "json",
    ...
    $(".addinput").blur(function(){
        ...
        if($(this).is("#imgCacheStatus")){
            if($("#imgCacheStatus").val()!=""){
                $(".spa3").text("");
                return true;
            }
        }
    ...
    function subForm() {
        ...
        if (ph.test($("#showId").val())  && $("#imgCacheName").val() != "" && $("#imgCacheStatus").val() != "" && $("#filename").val() != "") {
            var formData=new FormData($("#form0")[0]);
            formData.append('showId',$("#showId").val());
            formData.append('imgCacheName',$("#imgCacheName").val());
            formData.append('imgCacheStatus',$("#imgCacheStatus").val());
            formData.append('filename',$("#filename").val());
            $.ajax({
                url:"${pageContext.request.contextPath}/imgcache/addImgcache", //要处理的页面
