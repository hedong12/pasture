                ...
                <div class="row">
                    <div class="col-sm-12" style="margin: 0;padding: 0">
                        <div class="box">
                            ...
                            <input type="button"  class="modalLink btn btn-success" value="添加收益种类">
...
<%--                             添加表单 --%>
                            <div id="modal1" class="modal">
                                <div>
                                    <h2 style="margin: 15px;text-align: center;color: black">添加收益种类</h2>
                                </div>
                                <form id="form0" enctype="multipart/form-data">
                                    ...
                                    <label class="addlabel">种类名称：</label><input class="addinput" type="text"  id="szName" placeholder="请输入收益种类名称" /><span class="spa spa2"></span><br />
                                    <label class="addlabel">种类图片：</label>
                            ...
                            <div style="width: 50%;float: right;text-align: center;">
                                <form method="post" action="${pageContext.request.contextPath}/revenueType/revenueTypeList">
                            ...
                            <div class="item">
                                <div class="item-responsive ">
                                    <table class="table">
                                        ...
                                        <tbody>
                                        <c:forEach items="${pageInfo.list}" var="Rtype">
                                            <tr>
                                                <td>${Rtype.szName}</td>
                                                <td>
                                                    <img class="pic" height="40" width="60"  title="${Rtype.szName}" src="${pageContext.request.contextPath}/uploaded/${Rtype.szImg}">
                                                </td>

                                                </td>
                                                <td>
                                                     <a  onclick="delRTypeById('${Rtype.szId}')" href="javascript:;">
                                                         <span class="item-status delivered">删除</span>
                                                     </a>
                                                     <a id="updatePetsById" href="${pageContext.request.contextPath}/revenueType/toUpdateRevenueTypeById/${Rtype.szId}"><span class="item-status delivered">修改</span></a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                        </tbody><!--/tbody-->
                                ...
                                <div class="col-md-6">
                                    <nav aria-label="Page navigation">
                                        <ul class="pagination">
                                            ...
                                            <li><a href="${pageContext.request.contextPath}/revenueType/revenueTypeList?pn=1">首页</a></li>

                                            <!--上一页-->
                                            <li>
                                                <c:if test="${pageInfo.hasPreviousPage}">
                                                    <a href="${pageContext.request.contextPath}/revenueType/revenueTypeList?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                                            ...
                                            <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
                                                ...
                                                <c:if test="${page_num != pageInfo.pageNum}">
                                                    <li><a href="${pageContext.request.contextPath}/revenueType/revenueTypeList?pn=${page_num}">${page_num}</a></li>
                                                </c:if>
                                            </c:forEach>
                                            ...
                                            <li>
                                                <c:if test="${pageInfo.hasNextPage}">
                                                    <a href="${pageContext.request.contextPath}/revenueType/revenueTypeList?pn=${pageInfo.pageNum+1}"
                                                       aria-label="Next">
                                                        <span aria-hidden="true">»</span>
                                                    </a>
                                                </c:if>
                                            ...
                                            <li><a href="${pageContext.request.contextPath}/revenueType/revenueTypeList?pn=${pageInfo.pages}">尾页</a></li>
                                        </ul>
                                    </nav>
                                </div>
                            </div>
...
<script>
        ...
        //删除
        function delRTypeById(e) {
            var $this = $(this);
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/revenueType/delRTypeById",
                data: {"id" : e},
                dataType: "json",
    ...
    function subForm() {
        var ph = /^\d{1,10}$/;
        if (ph.test($("#showId").val()) &&  $("#szName").val() != "" && $("#filename").val() != "") {
            var formData=new FormData($("#form0")[0]);
            formData.append('showId',$("#showId").val());
            formData.append('szName',$("#szName").val());
            formData.append('filename',$("#filename").val());
            $.ajax({
                url:"${pageContext.request.contextPath}/revenueType/addRevenueTyper", //要处理的页面
