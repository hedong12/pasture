                ...
                <div class="row">
                    <div class="col-sm-12" style="margin: 0;padding: 0">
                        <div class="box">
                            ...
                            <input type="button"  class="modalLink btn btn-success" value="添加任务">
...
<%--                             添加表单 --%>
                            <div id="modal1" class="modal">
                                ...
                                <form id="form0" enctype="multipart/form-data">
                                    ...
                                    <label class="addlabel">任务名称：</label><input class="addinput" type="text"  id="taskName" placeholder="请输入任务名称" /><span class="spa spa2"></span><br />
                                    <label class="addlabel">任务内容：</label>
                                        <textarea class="addtextarea addinput" type="text"  id="taskContent" placeholder="请输入任务内容"></textarea>
                                    <span class="spa spa3"></span><br />
                                    <label class="addlabel">任务类型：</label>
                                    <select class="addinput" id="taskType">
                                        <option value="">
                                            请选择任务类型
                                        </option>
                                        <option value="0">
                                    ...
                                    <span class="spa spa6"></span>
                                     <br />
                                    <label class="addlabel">规则Id：</label>
                                    <input class="addinput" type="text"  id="guizeId" placeholder="请输入规则Id" />
                            ...
                            <div style="width: 50%;float: right;text-align: center;">
                                <form method="post" action="${pageContext.request.contextPath}/shouyiLedger/shouyiLedgerList">
                                    <input name="taskName" value="${taskName}" placeholder="请输入任务名">
                            ...
                            <div class="item">
                                <div class="item-responsive ">
                                    <table class="table">
                                        ...
                                        <tbody>
                                        <c:forEach items="${pageInfo.list}" var="ledger">
                                            <tr>
                                                <td>${ledger.username}</td>
                                                <td>${ledger.slNum}</td>
                                                <td>${ledger.szName}</td>
                                                <td>
                                                    <img class="pic" height="40" width="60"  title="${ledger.szName}" src="${pageContext.request.contextPath}/uploaded/${ledger.szImg}">
                                                </td>
                                                <td>
                                                     <a onclick="delLedgerById('${ledger.slId}')" href="javascript:;">
                                                         <span class="item-status delivered">删除</span>
                                                     </a>
                                                     <a href="${pageContext.request.contextPath}/task/toUpdateTaskById/${task.id}"><span class="item-status delivered">修改</span></a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                        </tbody><!--/tbody-->
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <nav aria-label="Page navigation">
                                        <ul class="pagination">
                                            <li><a href="${pageContext.request.contextPath}/shouyiLedger/shouyiLedgerList?pn=1">首页</a></li>
                                            <!--上一页-->
                                            <li>
                                                <c:if test="${pageInfo.hasPreviousPage}">
                                                    <a href="${pageContext.request.contextPath}/shouyiLedger/shouyiLedgerList?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                                            ...
                                            <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
                                                ...
                                                <c:if test="${page_num != pageInfo.pageNum}">
                                                    <li><a href="${pageContext.request.contextPath}/shouyiLedger/shouyiLedgerList?pn=${page_num}">${page_num}</a></li>
                                                </c:if>
                                            </c:forEach>
                                            <!--下一页-->
                                            <li>
                                                <c:if test="${pageInfo.hasNextPage}">
                                                    <a href="${pageContext.request.contextPath}/shouyiLedger/shouyiLedgerList?pn=${pageInfo.pageNum+1}"
                                                       aria-label="Next">
                                                        <span aria-hidden="true">»</span>
                                                    </a>
                                                </c:if>
                                            </li>
                                            <li><a href="${pageContext.request.contextPath}/shouyiLedger/shouyiLedgerList?pn=${pageInfo.pages}">尾页</a></li>
                                        </ul>
                                    </nav>
                                </div>
                            </div>
...
<script>
        ...
        //删除
        function delLedgerById(e) {
            var $this = $(this);
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/shouyiLedger/delLedgerById",
                data: {"id" : e},
                dataType: "json",
    ...
    $(".addinput").blur(function(){
        ...
        if($(this).is("#guizeId")){            // 排序判断
            var ph = /^\d{1,10}$/;
            if($("#guizeId").val()!=""){
                if(!(ph.test($("#guizeId").val()))){
    ...
    function subForm() {
        var ph = /^\d{1,10}$/;
        if (ph.test($("#showId").val()) && $("#taskName").val() != "" && $("#taskContent").val() != "" && ph.test($("#guizeId").val()) && $("#status").val() != "" && $("#taskType").val() != "") {
            var formData=new FormData($("#form0")[0]);
            formData.append('showId',$("#showId").val());
            formData.append('taskName',$("#taskName").val());
            formData.append('taskContent',$("#taskContent").val());
            formData.append('guizeId',$("#guizeId").val());
            formData.append('status',$("#status").val());
            formData.append('taskType',$("#taskType").val());
            $.ajax({
                url:"${pageContext.request.contextPath}/task/addTask", //要处理的页面
