                ...
                <div class="row">
                    <div class="col-sm-12" style="margin: 0;padding: 0">
                        <div class="box">
                            ...
                            <input type="button" id="alertForm"  class="modalLink btn btn-success" value="添加奖品">
...
<%--                             添加表单 --%>
                            <div id="modal1" class="modal">
                                <div>
                                    <h2 style="margin: 15px;text-align: center;color: black">添加经验奖励</h2>
                                </div>

                                <label class="addlabel">牧场id：</label><input class="addinput" type="text" id="showId" placeholder="请输入牧场ID" /><span class="spa spa1"></span><br />
                                <label class="addlabel">奖品名称：</label><input class="addinput" type="text"  id="expPrizeName" placeholder="请输入奖品名称" /><span class="spa spa2"></span><br />
                                <label class="addlabel">饲料：</label>
                                <select class="addinput" id="hotelNo">
                                    <option>请选择饲料</option>
                                </select><span class="spa spa3"></span><br />
                                <label class="addlabel">数量：</label><input class="addinput" type="text" id="num" placeholder="请输入数量" /><span class="spa spa4"></span><br />
                            ...
                            <div style="width: 50%;float: right;text-align: center;">
                                <form method="post" action="${pageContext.request.contextPath}/expprize/expPrizeList">
                                    <input id="fodderName" name="expPrizeName" value="${expPrizeName}" placeholder="请输入奖励名称...">
                            ...
                            <div class="item">
                                <div class="item-responsive ">
                                    <table class="table">
                                        ...
                                        <tbody>
                                        <c:forEach items="${pageInfo.list}" var="expprize">
                                            <tr>
                                                <td>${expprize.expPrizeName}</td>
                                                <td>${expprize.expName}</td>
                                                <td>
                                                    <img class="pic" height="40" width="60" title="${expprize.expName}" src="${expprize.expImg}">
                                                </td>
                                                <td>${expprize.num}</td>
                                                <td>
                                                    <fmt:formatDate value="${expprize.addTime}" pattern="yyyy-MM-dd" />

                                                </td>

                                                <td>
                                                     <a  onclick="delFodderById('${expprize.eId}')" href="javascript:;">
                                                         <span class="item-status delivered">删除</span>
                                                     </a>
                                                     <a id="updateExpPrizeById" href="${pageContext.request.contextPath}/expprize/toUpdateExpPrizeById?id=${expprize.eId}"><span class="item-status delivered">修改</span></a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                        </tbody><!--/tbody-->
                                ...
                                <div class="col-md-6">
                                    <nav aria-label="Page navigation">
                                        <ul class="pagination">
                                            ...
                                            <li><a href="${pageContext.request.contextPath}/expprize/expPrizeList?pn=1">首页</a></li>

                                            <!--上一页-->
                                            <li>
                                                <c:if test="${pageInfo.hasPreviousPage}">
                                                    <a href="${pageContext.request.contextPath}/expprize/expPrizeList?pn=${pageInfo.pageNum-1}" aria-label="Previous">
                                            ...
                                            <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
                                                ...
                                                <c:if test="${page_num != pageInfo.pageNum}">
                                                    <li><a href="${pageContext.request.contextPath}/expprize/expPrizeList?pn=${page_num}">${page_num}</a></li>
                                                </c:if>
                                            </c:forEach>
                                            ...
                                            <li>
                                                <c:if test="${pageInfo.hasNextPage}">
                                                    <a href="${pageContext.request.contextPath}/expprize/expPrizeList?pn=${pageInfo.pageNum+1}"
                                                       aria-label="Next">
                                                        <span aria-hidden="true">»</span>
                                                    </a>
                                                </c:if>
                                            ...
                                            <li><a href="${pageContext.request.contextPath}/expprize/expPrizeList?pn=${pageInfo.pages}">尾页</a></li>
                                        </ul>
                                    </nav>
                                </div>
                            </div>
...
<script>
        ...
        //删除
        function delFodderById(e) {
            var $this = $(this);
            $.ajax({
                type: "POST",
                url: "${pageContext.request.contextPath}/expprize/delExpPrizeById",
                data: {"id" : e},
                dataType: "json",
                success: function(data){
                    if (data.result == "true"){
    ...
    $(".addinput").blur(function(){
        ...
        if($(this).is("#hotelNo")){            // 饲料判断
            if($("#hotelNo").val()!="请选择饲料"){
                $(".spa3").text("");
                return true;
            }
        }

        if($(this).is("#expPrizeName")){            // 经验奖品判断
            if($("#expPrizeName").val()!=""){
                $(".spa2").text("");
                return true;
            }
    ...
    function subForm(petid) {
        var ph = /^\d{1,10}$/;
        if (ph.test($("#showId").val()) && ph.test($("#num").val()) && $("#hotelNo").val() != "请选择饲料" && $("#expPrizeName").val() != "") {
            var showId = $("#showId").val();
            var num = $("#num").val();
            var hotelNo = $("#hotelNo").val();
            var expPrizeName = $("#expPrizeName").val();
            $.ajax({
                url:"${pageContext.request.contextPath}/expprize/addExpPrize", //要处理的页面
                data:{"showId":showId,"num":num,"hotelNo":hotelNo,"expPrizeName":expPrizeName},  //要传过去的数据
                type:"POST",  //提交方式
                dataType:"json",
